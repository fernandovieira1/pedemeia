### DADOS PNAD -- AED
# - Análise exploratória dos dados de abandono escolar.
...
#   count(abandono) %>% 
#   summarise(
#     total_0 = sum(n[abandono == 0]),
#     total_1 = sum(n[abandono == 1]),
#     taxa_abandono = (total_1 / total_0) * 100
#   )
...
# Calcular os percentuais por idade e abandono
base_abandono_percentual <- base_abandono_pdm %>%
  group_by(V2009, abandono) %>%
  summarise(Contagem = n(), .groups = 'drop') %>%
  group_by(V2009) %>%
  mutate(Percentual = round(Contagem / sum(Contagem) * 100, 2))

# Recriar o gráfico com percentuais no topo das barras
ggplot(base_abandono_percentual, aes(x = as.factor(V2009), y = Contagem, fill = as.factor(abandono))) +
...
base_abandono_percentual_ano <- base_abandono_pdm %>%
  group_by(Ano, V2009, abandono) %>%
  summarise(Contagem = n(), .groups = 'drop') %>%
  group_by(Ano, V2009) %>%
  mutate(Percentual = round(Contagem / sum(Contagem) * 100, 2))

# Gráfico por ano com percentuais
ggplot(base_abandono_percentual_ano, aes(x = as.factor(V2009), y = Contagem, fill = as.factor(abandono))) +
  geom_bar(stat = 'identity', position = position_dodge(width = 0.9), color = 'black') +
  geom_text(aes(label = paste0(Percentual, '%')), 
            position = position_dodge(width = 0.9), vjust = -0.5, size = 3) +  # Inserir percentuais
  facet_wrap(~Ano, ncol = 2) +  # Facetar por ano
...
# Resumo estatístico: Contagem e proporção de Abandono por cor/raça
tabela_cor_raca <- base_abandono_filtrada %>%
  group_by(V2010, abandono) %>%
  summarise(Contagem = n(), .groups = 'drop') %>%
  group_by(V2010) %>%
...
# Calcular os percentuais de Abandono por cor/raça
base_abandono_percentual_cor <- base_abandono_filtrada %>%
  group_by(V2010, abandono) %>%
  summarise(Contagem = n(), .groups = 'drop') %>%
  group_by(V2010) %>%
  mutate(Percentual = round(Contagem / sum(Contagem) * 100, 2))
...
ggplot(base_abandono_percentual_cor, aes(x = V2010, y = Contagem, fill = as.factor(abandono))) +
  geom_bar(stat = 'identity', position = position_dodge(width = 0.9), color = 'black') +
  geom_text(aes(label = paste0(Percentual, '%')), 
...
# Calcular os percentuais de Abandono por cor/raça
base_abandono_percentual_cor_validos <- base_abandono_pdm %>%
  group_by(V2010, abandono) %>%
  summarise(Contagem = n(), .groups = 'drop') %>%
  group_by(V2010) %>%
  mutate(Percentual = round(Contagem / sum(Contagem) * 100, 2))
...
ggplot(base_abandono_percentual_cor_validos, aes(x = V2010, y = Contagem, fill = as.factor(abandono))) +
  geom_bar(stat = 'identity', position = position_dodge(width = 0.9), color = 'black') +
  geom_text(aes(label = paste0(Percentual, '%')), 
            position = position_dodge(width = 0.9), vjust = -0.5, size = 3.5) +
...
## 1.2.5A Exportação Final da Tabela (Sem NAs em abandono)** ####
# Criar uma tabela consolidada
tabela_cor_sem_na <- base_abandono_pdm %>%
  group_by(V2010, abandono) %>%
  summarise(Contagem = n(), .groups = 'drop') %>%
  group_by(V2010) %>%
  mutate(Proporcao = round(Contagem / sum(Contagem) * 100, 2)) %>%
...
# Calcular os percentuais por cor/raça, abandono e ano
base_abandono_percentual_ano <- base_abandono_pdm %>%
  group_by(Ano, V2010, abandono) %>%
  summarise(Contagem = n(), .groups = 'drop') %>%
  group_by(Ano, V2010) %>%
  mutate(Percentual = round(Contagem / sum(Contagem) * 100, 2)) %>%
  ungroup()
...
# Gráfico com os percentuais no topo das barras
ggplot(base_abandono_percentual_ano, aes(x = V2010, y = Contagem, fill = as.factor(abandono))) +
  geom_bar(stat = 'identity', position = position_dodge(width = 0.9), color = 'black') +
  geom_text(aes(label = paste0(Percentual, '%')), 
            position = position_dodge(width = 0.9), vjust = -0.5, size = 3.5) +
  facet_wrap(~Ano, ncol = 2) + # Facetar por ano
...
# Resumo estatístico: Contagem e proporção de Abandono por sexo
tabela_sexo <- base_abandono_filtrada %>%
  group_by(V2007, abandono) %>%
  summarise(Contagem = n(), .groups = 'drop') %>%
  group_by(V2007) %>%
  mutate(Proporcao = round(Contagem / sum(Contagem) * 100, 2)) %>%
  ungroup()
...
# Renomear colunas
colnames(tabela_sexo) <- c('Sexo', 'abandono', 'Contagem', 'Proporcao')
...
# Calcular os percentuais de Abandono por sexo
base_abandono_percentual_sexo <- base_abandono_filtrada %>%
  group_by(V2007, abandono) %>%
  summarise(Contagem = n(), .groups = 'drop') %>%
  group_by(V2007) %>%
  mutate(Percentual = round(Contagem / sum(Contagem) * 100, 2))
...
# Gráfico com os percentuais no topo das barras
ggplot(base_abandono_percentual_sexo, aes(x = V2007, y = Contagem, fill = as.factor(abandono))) +
  geom_bar(stat = 'identity', position = position_dodge(width = 0.9), color = 'black') +
  geom_text(aes(label = paste0(Percentual, '%')), 
            position = position_dodge(width = 0.9), vjust = -0.5, size = 3.5) +
...
# Calcular os percentuais de Abandono por sexo
base_abandono_percentual_sexo <- base_abandono_pdm %>%
  group_by(V2007, abandono) %>%
  summarise(Contagem = n(), .groups = 'drop') %>%
  group_by(V2007) %>%
  mutate(Percentual = round(Contagem / sum(Contagem) * 100, 2))
...
# Calcular os percentuais de Abandono por sexo por ano
base_abandono_percentual_sexo_ano <- base_abandono_pdm %>%
  group_by(Ano, V2007, abandono) %>%
  summarise(Contagem = n(), .groups = 'drop') %>%
  group_by(Ano, V2007) %>%
  mutate(Percentual = round(Contagem / sum(Contagem) * 100, 2))
...
# Resumo estatístico: Contagem e proporção de Abandono por região
tabela_regiao <- base_abandono_filtrada %>%
  group_by(regiao, abandono) %>%
  summarise(Contagem = n(), .groups = 'drop') %>%
  group_by(regiao) %>%
  mutate(Proporcao = round(Contagem / sum(Contagem) * 100, 2)) %>%
  ungroup()
...
# Renomear colunas
colnames(tabela_regiao) <- c('Regiao', 'abandono', 'Contagem', 'Proporcao')
...
base_abandono_percentual_regiao <- base_abandono_filtrada %>%
  group_by(regiao, abandono) %>%
  summarise(Contagem = n(), .groups = 'drop') %>%
  group_by(regiao) %>%
  mutate(Percentual = round(Contagem / sum(Contagem) * 100, 2))
...
# Gráfico com os percentuais no topo das barras
ggplot(base_abandono_percentual_regiao, aes(x = regiao, y = Contagem, fill = as.factor(abandono))) +
  geom_bar(stat = 'identity', position = position_dodge(width = 0.9), color = 'black') +
  geom_text(aes(label = paste0(Percentual, '%')), 
            position = position_dodge(width = 0.9), vjust = -0.5, size = 3.5) +
...
# Calcular os percentuais de Abandono por região
base_abandono_percentual_regiao <- base_abandono_pdm %>%
  group_by(regiao, abandono) %>%
  summarise(Contagem = n(), .groups = 'drop') %>%
  group_by(regiao) %>%
  mutate(Percentual = round(Contagem / sum(Contagem) * 100, 2))
...
tabela_regiao_ano <- base_abandono_filtrada %>%
  group_by(Ano, regiao, abandono) %>%
  summarise(Contagem = n(), .groups = 'drop') %>%
  group_by(Ano, regiao) %>%
  mutate(Proporcao = round(Contagem / sum(Contagem) * 100, 2)) %>%
  ungroup()
...
base_abandono_percentual_regiao_ano <- base_abandono_filtrada %>%
  group_by(Ano, regiao, abandono) %>%
  summarise(Contagem = n(), .groups = 'drop') %>%
  group_by(Ano, regiao) %>%
  mutate(Percentual = round(Contagem / sum(Contagem) * 100, 2)) %>%
...
ggplot(base_abandono_percentual_regiao_ano, aes(x = regiao, y = Contagem, fill = as.factor(abandono))) +
  geom_bar(stat = 'identity', position = position_dodge(width = 0.9), color = 'black') +
  geom_text(aes(label = paste0(Percentual, '%')),
            position = position_dodge(width = 0.9), vjust = -0.5, size = 3.5) +
  facet_wrap(~Ano, ncol = 2) +  # Facetar por ano
...
# Calcular os percentuais de Abandono por região e ano
base_abandono_percentual_regiao_validos_ano <- base_abandono_pdm %>%
  group_by(Ano, regiao, abandono) %>%
  summarise(Contagem = n(), .groups = 'drop') %>%
  group_by(Ano, regiao) %>%
  mutate(Percentual = round(Contagem / sum(Contagem) * 100, 2)) %>%
...
ggplot(base_abandono_percentual_regiao_validos_ano, aes(x = regiao, y = Contagem, fill = as.factor(abandono))) +
  geom_bar(stat = 'identity', position = position_dodge(width = 0.9), color = 'black') +
  geom_text(aes(label = paste0(Percentual, '%')),
            position = position_dodge(width = 0.9), vjust = -0.5, size = 3.5) +
  facet_wrap(~Ano, ncol = 2) +  # Facetar por ano
...
colnames(tabela_populacao) <- c('Populacao', 'abandono', 'Contagem', 'Proporcao')
...
ggplot(base_abandono_percentual_populacao, aes(x = V1022, y = Contagem, fill = as.factor(abandono))) +
  geom_bar(stat = 'identity', position = position_dodge(width = 0.9), color = 'black') +
  geom_text(
...
colnames(tabela_populacao_ano) <- c('Ano', 'Populacao', 'abandono', 'Contagem', 'Proporcao')
...
tabela_populacao_sem_na_ano <- tabela_populacao_ano %>%
  filter(!is.na(Populacao) & !is.na(abandono))

ggplot(tabela_populacao_sem_na_ano, aes(x = Populacao, y = Contagem, fill = as.factor(abandono))) +
  geom_bar(stat = 'identity', position = position_dodge(width = 0.9), color = 'black') +
  geom_text(
    aes(label = paste0(Proporcao, '%')),
    position = position_dodge(width = 0.9),
...
# Gráfico inicial: proporção de Abandono
ggplot(tabela_abandono, aes(x = '', y = Contagem, fill = as.factor(abandono))) +
...
# Exportar tabela limpa com stargazer
stargazer(tabela_abandono, type = 'text', summary = FALSE,
          title = titulo_dinamico,  # Título dinâmico
          digits = 2)
...
tabela_abandono_longitudinal <- base_abandono_filtrada %>% 
  ...
  group_by(Ano) %>%
  mutate(Proporcao = round((Contagem / sum(Contagem)) * 100, 2)) %>%
  filter(!is.na(abandono)) # Remover NAs de Abandono

tabela_abandono_longitudinal
...
# - base_abandono_filtrada: Dados de Abandono filtrados (T1 e T1+1)
# - base_abandono_pdm: base_abandono_filtrada reduzida ap público-alvo do pdm (14-24 anos)
