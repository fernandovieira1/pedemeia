...
head(base_evasao_filtrada$V2007)
...
stargazer(tabela_sexo_clean, type = "text", summary = FALSE,
          title = "Proporção de Evasão por Sexo (Sem NAs em Evasao)",
          digits = 2)
...
ggplot(tabela_rdpc_regiao_validos, aes(x = Faixa_RDPC, y = Contagem, fill = as.factor(evasao))) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.9), color = "black") +
  geom_text(
    aes(label = paste0(Proporcao, "%")),
...
# Gráfico com percentuais no topo (sem NAs) por cor, com texto rotacionado em 90 graus
...
## 1.8 Distribuição de RDPC por Gênero ####

## 1.9 Distribuição de RDPC por Escolaridade da Mãe ####

## 1.10 Distribuição de RDPC por Ensino Médio ####

## 1.11 Distribuição de RDPC por Evasão ####

## 1.12 Distribuição de RDPC por Evasão e Ensino Médio ####
